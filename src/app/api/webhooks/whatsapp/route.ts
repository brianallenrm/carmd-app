...
export async function POST(req: NextRequest) {
    try {
        ...
        // --- EVALUADOR INTELIGENTE DE APORTE DE DATOS (VENTANA DE 7 SEGUNDOS) ---
        // Si Mariana acaba de responder en los últimos 7s y el nuevo mensaje NO contiene datos clave (auto, año, km, placa, falla, o pregunta nueva),
        // mantenemos el mensaje en la memoria del historial pero no enviamos una respuesta duplicada.
        let shouldSuppressDuplicateReply = false;
        if (chat && chat.chatHistory) {
            try {
                const historyList = JSON.parse(chat.chatHistory);
                if (Array.isArray(historyList) && historyList.length > 0) {
                    ...
                    if (lastAssistantMsg && lastAssistantMsg.timestamp) {
                        ...
                        if (timeDiffSec < 7) {
                            ...
                            const isCourtesyFiller = 
                                textClean === 'por favor' || textClean === 'porfavor' || textClean === 'porfa' ||
                                textClean === 'ok' || textClean === 'va' || textClean === 'gracias' ||
                                textClean === 'muchas gracias' || textClean === 'saludos' || textClean === 'hola' ||
                                textClean === 'buenas tardes' || textClean === 'buenos dias' || textClean === 'buenas noches' ||
                                textClean === 'disculpa' || textClean === 'perdon';

                            const hasNumbers = /\d+/.test(text);

                            const hasCarBrandOrModel = /\b(aveo|beat|camry|prius|vento|jetta|sentra|tsuru|corolla|civic|march|spark|rio|versa|fit|crv|rav4|suburban|tahoe|silverado|hilux|ram|f150|kicks|tracker|groove|grove|polo|golf|cx3|cx5|cx30|mazda|toyota|nissan|chevrolet|ford|honda|volkswagen|vw|kia|hyundai|bmw|audi|mercedes|jeep|dodge)\b/i.test(text);

                            const hasServiceOrProblem = /\b(afinacion|frenos|aceite|humo|calent|ruido|vibr|balatas|suspension|bateria|llanta|sensor|check|engine|airbag|luz|testigo|fuga|clima|aire|transmision|caja)\b/i.test(text);

                            if (isCourtesyFiller || (!hasNumbers && !hasCarBrandOrModel && !hasServiceOrProblem)) {
                                shouldSuppressDuplicateReply = true;
                            }
                        }
                    }
                }
            } catch (e) {
                console.error("Error en Evaluador Inteligente de Aporte de Datos:", e);
            }
        }

        if (shouldSuppressDuplicateReply) {
            console.log(`[Smart Evaluator] Omitiendo respuesta para mensaje sin datos técnicos nuevos ("${text}") dentro de la ventana de 7s.`);
            return new NextResponse('OK (Smart Evaluator Suppressed)', { status: 200 });
        }
...
1. 'respuesta_whatsapp': 
    - PERSONALIZACIÓN OBLIGATORIA CON NOMBRE Y VEHÍCULO: Si en los 'datos acumulados hasta el momento en tu memoria' ya tienes un campo 'name' (distinto de '...') y/o un campo 'vehicle' (distinto de '...'), ES ESTRICTAMENTE OBLIGATORIO que te dirijas al cliente llamándolo por su nombre de pila (ej: "¡Gracias, Marco Antonio!" o "¡Perfecto, Marco Antonio!") y que menciones su auto específico en tu respuesta (ej: "Encontré tu expediente registrado para tu Hyundai Elantra 2019..."). NUNCA envíes respuestas genéricas u omitas su nombre/auto cuando ya están cargados en tu memoria acumulada.
    ...
    - Si el cliente menciona que ya es cliente de CarMD o pregunta por su historial/expediente de mantenimiento o servicios pasados: En el PRIMER mensaje, dile amablemente que por seguridad no ves su historial por chat, dale las 2 opciones (asesor o agendar) y PÍDELE ÚNICAMENTE las *placas de su vehículo*. En el SEGUNDO mensaje (cuando da la placa), si en tu memoria ya se cargaron los datos del cliente (campo 'name' distinto de '...'), SALÚDALO OBLIGATORIAMENTE por su nombre (ej: "¡Gracias, Marco Antonio!"), confirma su vehículo (ej: "Encontré tu expediente registrado para tu Hyundai Elantra 2019...") e indícale que el equipo de asesores ya tiene su recado. Si no se encuentran las placas (nombre sigue en '...'), pídele amablemente su nombre completo y modelo de auto. NUNCA uses la frase "Detendré mis respuestas automáticas" ni intentes desactivarte.
 ...
 3. 'cita_lista_para_resumen':
    ...
    - MANEJO INTELIGENTE DE CORREO: NUNCA inventes o asumas "N/A" para el correo electrónico ('email') si aún no se lo has solicitado al cliente en la conversación. Pídele amablemente su correo electrónico. Si el cliente proporciona su correo, guárdalo en 'email'. Si el cliente responde explícitamente que no tiene correo o que no lo usa (ej: "no tengo correo", "no uso email"), guarda 'email' como "Sin correo" y avanza normalmente. Si el campo 'email' sigue estando en "...", pon 'cita_lista_para_resumen' en \`false\` y solicita su correo.
...
Recuerda: Eres un JSON válido. No uses markdown de código, devuelve únicamente el JSON plano y parseable.`;
            ...
            let hasRequiredFieldsForSummary = structuredOutput.cita_lista_para_resumen === true &&
                                              ...
                                              mergedParams.email && mergedParams.email !== '...' && mergedParams.email !== 'N/A' &&
